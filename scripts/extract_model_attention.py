...
def extract_attention(method, gaze_dataset, model_name, out_path, layer, rollout, lowercase):
    ...
    if method == 'valuezeroing':
        attn_extractor = ValueZeroingContributionExtractor(model_name, layer, rollout, 'first')
    elif method == 'alti':
        attn_extractor = AltiContributionExtractor(model_name, layer, rollout, 'first')
    else:
        attn_extractor = AttentionMatrixExtractor(model_name, layer, rollout, 'first')
...
def main():
    ...
    training_modes = ['pretrained_not_finetuned', 'pretrained_finetuned', 'not_pretrained_finetuned']
    languages = ['en', 'it']

    eye_tracking_data_dir = 'augmenting_nlms_meco_data/'
    finetuned_models_dir = '/home/lmoroni/__workdir/augmenting_nlms_meco/output'
    attn_results_dir = 'output/attn_data/'
    ...
    for language in languages:
        ...
        for file_name in [file_name for file_name in os.listdir(language_src_dir) if
                          file_name != 'all_mean_dataset.csv']:
                ...
                for training_mode in training_modes:
                    ...
                    for layer in range(12):
                        ...
                        if not os.path.exists(out_path):
                            extract_attention(args.method, gaze_dataset, model_path, out_path, layer, rollout, lowercase)
